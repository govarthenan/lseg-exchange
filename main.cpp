...
vector<Order> readCSVFile(const string& filename)
{
    ...
    string possible_instruments[] = { "Rose", "Lavender", "Lotus", "Tulip",
        "Orchid" }; // For validation
    ...
    while (
        data_file.good()) { // When filestream is being read, iterate each line
        ...
        valid_instrument = 0; // For each new row, set the indicator as 0,
                              // since it's unchecked
...
{
    // Read CSV
    string file_name = ".\\orders.csv"; // hi
    vector<Order> orders = readCSVFile(file_name); // hello
}